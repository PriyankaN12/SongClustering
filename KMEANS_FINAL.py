...
def lsh_cluster_updater(clusters):
    for cluster in clusters:
        ...
        # print("sign_length",signature_length)
        number_of_bands=50
        for song in cluster.members:
            chunk_size = signature_length / number_of_bands
            # song_signature = song.signature.split(",")
...
#
# def update_each_cluster(cluster_id):
#     cluster = cen[0]
#     for i in cen:
#         if i.id == cluster_id:
#             cluster = i
...
while flag<1 and iter_counter<10:
#Cluster assignment
...
#Cluster updation
    cid=set()
    # parallel_updation=sc.parallelize(cen,G_K).mapPartitions(jaccard_cluster_updater)

    # parallel_updation = sc.parallelize(cen, partition_size).mapPartitions(lsh_cluster_updater).groupByKey().mapValues(lambda x:list(x)).partitionBy(partition_size).mapPartitions(find_candidates)\
    ...
    # print("PU",[i.id for i in collector])
    # print("PU", len(collector))
    # print("PUC",collector[0])
    # cen=parallel_updation.collect()
    # cen=collector
...
for id in old_cen:
    ...
    # print("laza",list_cluster_elements)
